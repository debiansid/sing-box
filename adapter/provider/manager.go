...
func (m *Manager) Initialize(startOutbounds func() error) {
	m.startOutbounds = startOutbounds
}
...
func (m *Manager) Start(stage adapter.StartStage) error {
	...
	if stage == adapter.StartStateStart && len(providers) > 0 {
		...
		for _, provider := range providers {
			...
			if m.startOutbounds != nil {
				if err := m.startOutbounds(); err != nil {
					return E.Cause(err, "start loaded provider outbounds")
				}
			}
		}
		return nil
	...
	if stage == adapter.StartStatePostStart {
		for _, provider := range providers {
			if err := postStartProvider(provider); err != nil {
				return E.Cause(err, stage, " provider/", provider.Type(), "[", provider.Tag(), "]")
			}
		}
	}
...
func (m *Manager) Create(ctx context.Context, router adapter.Router, logFactory log.Factory, tag string, providerType string, options any) error {
	...
	if m.started {
		...
		if m.stage >= adapter.StartStatePostStart {
			if err = postStartProvider(provider); err != nil {
				common.Close(provider)
				return E.Cause(err, "post-start provider/", provider.Type(), "[", provider.Tag(), "]")
			}
		}
	}
...
func postStartProvider(provider adapter.Provider) error {
