	...
	for i, opt := range newOpts {
		...
		if !exist || !active || !reflect.DeepEqual(opt, oldOptByTag[tag]) {
			...
			if err != nil {
				...
				if active {
					if closeErr := removeProviderMember(a.outbound, owned); closeErr != nil {
...
func (a *Adapter) Close() error {
	...
	for _, ob := range outbounds {
		if err2 := removeProviderMember(a.outbound, ob); err2 != nil {
	...
	for _, ep := range endpoints {
		if err2 := removeProviderMember(a.endpoint, ep); err2 != nil {
			err = E.Append(err, err2, func(err error) error {
	...
	for i, opt := range newOpts {
		...
		if !exist || !active || !reflect.DeepEqual(opt, oldOptByTag[tag]) {
			...
			if err != nil {
				...
				if active {
					if closeErr := removeProviderMember(a.endpoint, owned); closeErr != nil {
...
func (a *Adapter) removeUselessEndpoints(newTags []string) {
	...
	for _, ep := range snap {
		tag := ep.Tag()
		if !exists[tag] {
			if err := removeProviderMember(a.endpoint, ep); err != nil {
...
func (a *Adapter) removeUseless(newTags []string) {
	...
	for _, opt := range snap {
		if !exists[opt.Tag()] {
			if err := removeProviderMember(a.outbound, opt); err != nil {
