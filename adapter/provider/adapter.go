...
func (a *Adapter) UpdateOutbounds(oldOpts []option.Outbound, newOpts []option.Outbound) {
	...
	oldTags := a.deduplicateOutboundTags(oldOpts)
	newTags := a.deduplicateOutboundTags(newOpts)
	var (
	...
	for i, opt := range newOpts {
		...
		if !exist || !reflect.DeepEqual(opt, oldOptByTag[tag]) {
...
func (a *Adapter) UpdateEndpoints(oldOpts []option.Endpoint, newOpts []option.Endpoint) {
	...
	oldTags := a.deduplicateEndpointTags(oldOpts)
	newTags := a.deduplicateEndpointTags(newOpts)
	var (
...
func (a *Adapter) deduplicateOutboundTags(opts []option.Outbound) []string {
	tagCount := make(map[string]uint16)
	tags := make([]string, 0, len(opts))
...
func (a *Adapter) deduplicateEndpointTags(opts []option.Endpoint) []string {
	tagCount := make(map[string]uint16)
	tags := make([]string, 0, len(opts))
	for i, opt := range opts {
		var tag string
