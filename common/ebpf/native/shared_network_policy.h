...
INLINE bool ipv4_policy_selected(
    ...
    if (dns_policy != SB_SHARED_POLICY_CONTINUE &&
        dns_policy != SB_SHARED_POLICY_RESPECT_SOURCE) return dns_policy == SB_SHARED_POLICY_PROXY;
    if (!ipv4_client_selected(source_mac, source, control)) return false;
    if (dns_policy == SB_SHARED_POLICY_RESPECT_SOURCE) return true;
