...
func TestTCEndpointTriStateIntegration(t *testing.T) {
	...
	for _, key := range []uint32{tcEndpointStatForceIntercept, tcEndpointStatNativeBypass} {
		var perCPU []uint64
		if err := backend.runtime.maps["tc_endpoint_stats"].Lookup(&key, &perCPU); err != nil {
			t.Fatalf("read endpoint counter %d: %v", key, err)
		}
		var total uint64
		...
		if total != 3 {
			t.Fatalf("unexpected endpoint counter %d: got %d, want 3", key, total)
		}
	}
}
