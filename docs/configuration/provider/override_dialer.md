...
When `detour` is configured, it overrides the subscription's original value. When omitted, the original value is preserved. Setting it explicitly to `""` clears the subscription's `detour`.
