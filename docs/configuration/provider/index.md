...
The node `node_name` from `provider` retains its original tag `node_name`.

Node tags share the outbound and endpoint namespaces. A node whose tag is already owned by another provider or a static configuration is skipped with a warning; it does not replace the existing node.
...
The cache stores the original subscription before filtering and overrides. Current provider settings are applied again when loading it. Older caches that stored processed nodes require one fresh download after upgrading.
