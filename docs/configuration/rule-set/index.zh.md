...
Clash API 的 `ruleCount` 统计 `domain`、`domain_suffix`、`domain_keyword`、
`domain_regex`、AdGuard 域名规则、`source_ip_cidr` 和 `ip_cidr` 中的条目数。
每条计为 1，CIDR 不展开为单个 IP 地址。端口等附加条件不会减少该数量。
逻辑规则递归累加子规则的数量，规则集累加顶层规则的数量。
不含域名或 IP 条目的规则保留原有的条件分组计数方式。

源码列表按提供的条目计数，包括重复项。二进制和内存映射规则集统计保留下来的域名条目和
规范化后的 CIDR 前缀。编译可能去重并合并 IP 网段，因此数量可能少于原始源码。
不同规则中的条目分别计数。
...
对于远程规则集，优先根据 URL 的扩展名推断格式，其次使用路径的扩展名。显式指定的 `format` 优先于两者。
...
对于远程规则集，下载内容存储于该路径。启用规则集缓存后，`cache.db` 也会保存内容和元数据。如果文件与数据库内容不同，则使用数据库中的有效内容，并在下次更新时重新下载以修复文件。路径留空时，内容仅存储于 `cache.db`。
