...
func NewURLTest(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.URLTestOutboundOptions) (adapter.Outbound, error) {
	outbound := &URLTest{
		...
		includeOrder:    buildIncludeOrder(options.IncludeOrder),
...
func (s *URLTest) onProviderUpdated(tag string) error {
	...
	var providerOutbounds []adapter.Outbound
	for _, providerTag := range s.providerTags {
		if providerTag != tag && s.outboundsCache[providerTag] != nil {
			providerOutbounds = append(providerOutbounds, s.outboundsCache[providerTag]...)
			continue
		}
		provider := s.providers[providerTag]
		cache := filterProviderOutbounds(provider.Outbounds(), s.exclude, s.include)
		providerOutbounds = append(providerOutbounds, cache...)
		s.outboundsCache[providerTag] = cache
	}
	sortProviderOutbounds(providerOutbounds, s.includeOrder)
	for _, detour := range providerOutbounds {
		tags = append(tags, detour.Tag())
	}
	outbounds = append(outbounds, providerOutbounds...)
	if len(tags) == 0 {
		detour, _ := s.outbound.Outbound("Compatible")
		tags = append(tags, detour.Tag())
		outbounds = append(outbounds, detour)
