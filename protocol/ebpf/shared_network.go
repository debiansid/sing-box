...
func (s *sharedNetwork) runFlowJanitor(ctx context.Context, done chan<- struct{}) {
	...
	for {
		...
		if !sharedFlowSweepRequired(now.Sub(lastSweep), pressure, reservationPressure) {
			continue
		}
		...
		result, err := backend.SweepOrphanedFlows(maxIdle)
...
func sharedFlowSweepRequired(elapsed time.Duration, pressure bool, reservationPressure bool) bool {
	return pressure || reservationPressure || elapsed >= sharedFlowSweepInterval
}
