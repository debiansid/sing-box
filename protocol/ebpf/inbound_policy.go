...
func (i *Inbound) refreshBypassRuleSetsLocked(extractRuleSets bool, warnEmpty bool, logRuleSetCount bool) (bool, error) {
	if !extractRuleSets {
		warnEmpty = false
		logRuleSetCount = false
	}
	...
	if backend != nil {
		...
		if i.sharedNetwork != nil {
			if sharedBackend := i.sharedNetwork.sharedBackendInstance(); sharedBackend != nil {
				if i.sharedNetwork != nil && sharedBackend.IndependentBypassCIDR() {
					_, err = sharedBackend.UpdateBypassCIDR(sharedPrefixes)
				} else {
