...
func TestSharedFlowSweepRequired(t *testing.T) {
	if sharedFlowSweepRequired(sharedFlowSweepInterval-time.Second, false, false) {
		t.Fatal("normal shared flow sweep ran early")
	}
	if !sharedFlowSweepRequired(sharedFlowSweepInterval, false, false) {
		t.Fatal("normal shared flow sweep did not run on schedule")
	}
	if !sharedFlowSweepRequired(time.Second, true, false) {
		t.Fatal("map pressure did not request an early sweep")
	}
	if !sharedFlowSweepRequired(time.Second, false, true) {
