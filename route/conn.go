...
func (m *ConnectionManager) NewConnection(ctx context.Context, this N.Dialer, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
	...
	if len(metadata.DestinationAddresses) > 0 || metadata.Destination.IsIP() {
		if m.concurrentDial {
			remoteConn, err = dialer.DialConcurrentNetwork(ctx, this, m.logger, N.NetworkTCP, metadata.Destination, metadata.DestinationAddresses, metadata.NetworkStrategy, metadata.NetworkType, metadata.FallbackNetworkType, metadata.FallbackDelay)
		} else {
