...
func TestEndpointPortSchemaBounds(t *testing.T) {
	generated, err := schema.Generate(context.Background(), reflect.TypeFor[EBPFLocalOptions]())
	if err != nil {
		t.Fatal(err)
	}
	...
	endpoint := document["$defs"].(map[string]any)["EBPFEndpointConnectedBypassOptions"].(map[string]any)
	...
	for _, variant := range port["anyOf"].([]any) {
		...
		if node["minimum"] != float64(1) || node["maximum"] != float64(65535) {
			t.Fatalf("endpoint port bounds: %+v", node)
		}
	}
}
