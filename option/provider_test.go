...
func TestProviderRejectsRemovedOverrides(t *testing.T) {
	for _, content := range []string{
		`{"url":"https://example.com/provider","override_anytls":{"disable_reuse":true}}`,
		`{"url":"https://example.com/provider","override_tls":{"certificate_server_name":"example.com"}}`,
	} {
		var options ProviderRemoteOptions
		require.Error(t, json.Unmarshal([]byte(content), &options))
	}
}
